...
- **继续已有会话**：在手机上浏览最近的 Codex 会话、查看 Markdown 历史、预览项目图片，并发送新的文字或图片消息。
...
- **预览 Codex 可视化**：Codex 生成的本机 HTML 交互稿可在全屏、禁用网络的浏览器沙箱中预览，也可下载原文件。
...
| 文件访问 | 位图预览只能来自配置根目录，并经过内容校验、缩放和 WebP 转换；SVG 只允许下载。Codex HTML 可视化受大小限制，并在隔离且禁用网络的框架中运行。原文件下载仍需明确确认，并使用随机、绑定当前客户端且短时有效的能力凭证。 |
