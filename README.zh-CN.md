...
- **跟随真实进度**：查看运行中和完成未读的任务，并跟踪真实活动、计划步骤与文件变更统计，不展示原始工具输出。
...
- **长会话也能快速打开**：优先打开最近内容，向上滚动时继续加载历史，并可复制带完成时间的消息。
...
你需要一台可访问的 Linux ECS/VPS，以及一台已经运行 Codex Desktop/CLI 的 Windows 电脑。轻量
转发主机即可；不需要数据库、本机公网 IP 或家庭网络入站端口。域名、证书和反向代理都是可选方案。
代码支持 WS 和 WSS，但经过不可信网络时优先使用 WSS、VPN 或安全隧道。
...
随后在运行 Codex 的电脑上安装连接器，通过 `./scripts/relay.sh approve` 批准它，再用
`./scripts/relay.sh pair <公网地址>` 生成浏览器配对链接。精简后的
[部署指南](docs/deployment.zh-CN.md)包含完整四步流程、入口选择、升级方式和少量可选参数。

不要把参考部署的 3300 端口暴露到公网，也不要把项目复制到 ECS。发布服务前请阅读
