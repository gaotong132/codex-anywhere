...
- **随时补充下一步**：Web 持有的任务运行中可追加指令；桌面持有的任务运行中可排队一条文字指令，且不会新建会话。
- **跟随执行进度**：查看哪些会话正在运行，并及时了解有价值的工作进展。
- **完成后及时提醒**：可主动开启完成和待审批通知；浏览器与手机系统支持时，页面断开后也可通过 Web Push 提醒。
...
浏览器和本机连接器都会主动连接转发服务。转发服务负责鉴权，并要求双方使用完全一致的当前协议和
能力集合。协议 v3 强制所有应用请求、响应、事件、预览和下载分块进入经过身份验证的端到端加密通道；
系统不存在明文应用帧回退路径，版本不一致的节点会被拒绝，必须同步升级。

新会话和符合条件的空闲会话由连接器通过 Codex app-server JSON-RPC 协议持有，因此可以接收
原生增量事件并在浏览器处理审批。已经在 Codex Desktop 中运行的会话仍由桌面持有，通过任务
工具投递消息并在同一条 WebSocket 上自适应轮询历史尾部。桌面仍持有写入者时，连接器可以在内存中
保留一条纯文字的下一轮指令，并在写入者释放后启动；重启连接器会取消这条待执行指令。已经在桌面
产生的待审批请求仍需在 Desktop 中处理。Codex Anywhere 没有实现 ACP。

通知刻意与加密会话通道分离：在线但位于后台的页面由浏览器在本地生成通知；页面断开后，连接器只向
转发服务发送 `completed` 或 `approval` 类别，不包含会话标题、消息正文、项目名或文件路径。
...
| 防护层 | 当前实际实现 |
| --- | --- |
| 设备访问 | 浏览器优先使用十分钟内有效、仅可使用一次的配对链接，并持久化 Ed25519 设备密钥。转发服务只保存尚未使用链接的校验值；完成配对后，浏览器使用已批准设备密钥重连，不再依赖共享 Token。连接器和恢复 Token 流程仍使用独立作用域和随机质询。 |
| 内容保护 | 协议 v3 强制浏览器与连接器使用已批准的 Ed25519 身份验证临时 X25519 握手，再用 XChaCha20-Poly1305 加密有序应用帧。转发服务拒绝明文应用帧，只能看到路由元数据、时间和密文大小，看不到消息或文件内容。 |
...
| 文件访问 | 位图预览只能来自配置根目录，并经过内容校验、缩放和 WebP 转换；SVG 只允许下载。Codex HTML 可视化受大小限制，在浏览器解密，并在隔离且禁用网络的框架中运行。原文件下载仍需明确确认，并使用随机、绑定当前客户端且短时有效的能力凭证。 |
| 转发部署 | 参考 Compose 只绑定 ECS 回环地址；容器使用非 root 用户、只读文件系统并删除全部 Linux capabilities；只持久化设备公钥、审批元数据和用户主动开启的 Web Push 端点，不保存会话或文件内容。 |
| 浏览器加固 | 连接前从地址栏移除一次性密钥片段，批准后清理临时配对/Token 数据；WebSocket 要求同源；网页响应包含严格 CSP 和其他浏览器安全响应头。 |

安全边界也必须如实说明。端到端加密可以避免正常运行的转发进程读取当前应用帧，但不能把部署变成
零信任系统。ECS 会提供 Web 代码并管理设备信任注册表；宿主机或 root 管理员被攻破后，仍可能下发
被修改的浏览器代码、改变后续信任关系、注册攻击者控制的设备或观察元数据。浏览器设备私钥保存在浏览器
配置中，而不是硬件安全存储；浏览器配置或恶意扩展被攻破时，攻击者可以冒充该设备。本机电脑被攻破
后，攻击者也能访问 Codex 可访问的数据。代码支持不含 TLS 的 `ws://`，应用帧仍保持端到端加密，但
Web 页面、认证引导和元数据无法抵御网络攻击。开启 Web Push 后，转发服务与浏览器所用推送服务还会知道一次通用的完成或待审批
事件及其发生时间，但不会收到对应会话内容；不受信网络仍应使用 WSS、VPN 或安全隧道。
...
5. 生成一个仅可使用一次的浏览器配对链接，把示例地址替换为实际 Web 入口：
   ...
   上传的二维码截图。共享浏览器 Token 仍可用于管理员恢复，但不再是日常登录的首选方式。
...
| `BRIDGE_PUSH_SUBJECT` | 转发服务 | 可选 Web Push 联系 URI；Compose 会在受保护状态卷内自动生成 VAPID 密钥 |
| `BRIDGE_URL` | 连接器 | 转发服务 WebSocket 地址，支持 `ws://` 和 `wss://` |
| `CODEX_UI_LANGUAGE` | 转发服务 | Web 界面语言：`zh-CN` 或 `en` |

完成配对后，浏览器使用已批准、持久化的 Ed25519 设备密钥签署每次随机质询。一次性配对和恢复
Token 登录会把 HMAC 证明与设备签名绑定到同一质询，因此截获后不能重放。应用帧在 `ws://` 上仍会
端到端加密，但普通 HTTP/WS 无法保护 Web 代码分发、配对引导和元数据；不受信网络仍应使用
`wss://`、VPN 或其他安全隧道。
