...
- **继续真实 Codex 会话**：浏览和重命名最近会话，读取 Markdown 历史，发送文字或图片，也可以在已有
  本机项目中新建任务。名称会写回当前执行节点上的原会话。
- **跟随执行过程**：查看运行中和完成未读状态，以及进度、计划步骤、工具目的、执行时间和文件变更；
  ...
  长会话先打开最新一页，只有用户向上浏览后才继续加载更早记录，并保持当前阅读位置；加载失败可见且可重试。
  自动任务报告保留完整正文与精简来源标识，末尾 heartbeat 内部字段和重复通知摘要不进入展示或复制内容。
- **随时补充下一步**：连接器持有的运行中任务可以追加文字；已有 Desktop 会话在支持时使用桌面投递。
  点击运行状态条可查看耗时；只有当前连接器确实持有该轮次时才会显示停止按钮。消息直接发送，Web 端
  不维护排队队列。
- **配置 Codex**：查看或修改模型、思考强度、快速模式和审批模式。无头连接器支持“请求批准”、
  “帮我批准”和由执行节点显式开放的“完全访问权限”；当前选择和新会话默认值按执行环境隔离。
  所选连接器的 Codex 模型目录提供 GPT-6 Astra（`gpt-6-astra`）时即可选择；思考档位以该环境返回值为准，
  包括宿主提供的 Max、Ultra。模型可用性参见 [Codex 官方模型说明](https://learn.chatgpt.com/docs/models)。
...
支持 PNG、JPEG、WebP，每条消息一张图片；再次粘贴会替换待发送图片。普通文字仍可正常粘贴，
也可以继续通过“＋”选择图片文件。浏览器插件侧栏复用同一功能。
...
| SVG | 默认显示图形，可切换源码；本地图片引用和 `svg` 代码块也支持渲染 |
| 常见源代码和配置文件 | 识别语言并按需加载语法着色器的代码预览 |
| 纯文本、日志、CSV、TSV | 支持横向滚动的纯文本预览 |
| 二进制、敏感或未识别文件 | 继续使用原有的确认下载流程，不做内联预览 |

SVG 使用隔离的图片上下文渲染，保留矢量细节，不执行脚本或加载外部资源。建议将样式和图片嵌入 SVG；
格式损坏时仍可查看源码和下载。本地文件链接支持 Windows 和 Linux 连接器路径。

文本预览可以读取所选连接器服务账号可访问的任意绝对路径，不再受配置根目录限制；仍只接受不超过 2 MiB 的普通 UTF-8 文件。`.env`、`.pem`、`.key` 等敏感
...
压缩过程与完成状态共用一个时间线标记：**正在压缩上下文**及等待计时在完成后原位变为**上下文已压缩**，
保持相同样式和位置。Headless 运行使用原生
`contextCompaction` 生命周期；桌面任务的记录只包含完成事件，因此通过有界、只读的本地 Codex 日志查询
识别当前任务、当前轮次的压缩阶段。日志数据库缺失或格式变化时，聊天仍正常工作，仅保留完成节点；
不会向浏览器传输日志正文、估算百分比或提示词。需更新 Connector 与 Web，无需重载扩展或重新配对。
