...
连接器通过 Codex app-server JSON-RPC 管理自己持有的会话；正在 Desktop 中运行的会话使用任务投递
和自适应历史轮询。Web 持有的任务支持追加指令，Desktop 持有的任务可以排队一条下一轮文字指令。
Desktop 已持有的审批仍在 Desktop 处理。Codex Anywhere 没有实现 ACP。
...
Web 代码或批准记录并观察元数据；浏览器配置或连接器电脑被攻破后，也会继承该端权限。Web Push 只
暴露通用事件类型和时间。直接 `ws://` 仍会加密应用流量，但不能保护 Web 分发、配对和元数据；经过
