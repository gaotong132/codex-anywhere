...
- **设备级二次认证**：Token 只是第一层；每个浏览器和连接器还必须使用自己持久化的 Ed25519
  设备密钥签名，并经过可信设备批准，已批准设备也可随时撤销。
...
4. 在手机浏览器中打开转发服务地址并输入浏览器 Token。浏览器会自动生成设备身份，并显示等待
   批准的设备 ID。
5. 首次安装时，在 ECS/VPS 上核对并按[正式部署指南](docs/deployment.md)的一次性运维步骤批准这个
   ID；之后的新浏览器和连接器可以直接由已有可信浏览器批准或撤销。
