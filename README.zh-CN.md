...
当前版本的两端会建立经过身份验证的端到端加密通道，转发服务只路由密文；Codex 执行和文件访问仍在
本机完成。滚动升级期间，只有一端尚未声明端到端加密能力时才保留旧版明文兼容路径。
...
| 内容保护 | 当前浏览器与连接器使用已批准的 Ed25519 身份验证临时 X25519 握手，再用 XChaCha20-Poly1305 加密有序应用帧。转发服务能看到路由元数据、时间和密文大小，但看不到消息或文件内容。 |
...
被修改的浏览器代码、改变后续信任关系、观察元数据或强制降级到旧协议。浏览器设备私钥保存在浏览器
配置中，而不是硬件安全存储；浏览器配置或恶意扩展被攻破时，攻击者可以冒充该设备。本机电脑被攻破
后，攻击者也能访问 Codex 可访问的数据。代码支持明文 `ws://`，但它不能保护 Web 页面、认证引导和
元数据免受网络攻击。开启 Web Push 后，转发服务与浏览器所用推送服务还会知道一次通用的完成或待审批
