...
- Codex、项目、附件和生成文件都留在连接器电脑上；该电脑不接受公网入站连接。
- 转发服务没有会话数据库，也不会有意持久化消息、预览、可视化或下载分块，只保存设备信任状态。
- 浏览器通过十分钟单次链接注册，之后使用已批准的 Ed25519 密钥认证；不存在共享浏览器 Token 或
  恢复登录。
- 连接器同时需要连接器专用密钥和管理员批准的 Ed25519 身份。Windows 使用当前用户 DPAPI 保护
  两类连接器凭据。
...
- Markdown 预览必须由用户点击触发，只接受不超过 2 MiB 的 UTF-8 `.md` 或 `.markdown` 文件，并始终
  限制在配置或受管的本机根目录内。预览界面保留确认下载入口，文件内容不会以明文经过转发服务。
- Mermaid 代码块按需加载渲染器，启用严格安全模式并限制输入大小和边数；生成的 SVG 作为不可执行图片
  显示，语法错误时回退为源代码。
- 交互式 HTML 仅限 Codex 可视化目录，并在不透明来源、禁止联网的沙箱中渲染。
...
可以替换后续 Web 代码或信任记录、批准恶意设备、观察元数据或拒绝服务。连接器电脑或已批准浏览器
配置被攻破后，也会继承该端权限。端到端加密能够减少转发服务暴露面，但不能把部署变成零信任架构。
...
2. 连接器密钥泄露时，更换 `BRIDGE_CONNECTOR_TOKEN`，重新安装 Windows 连接器凭据，并重启转发
   服务和连接器。
