...
- 本机文本预览必须由用户点击触发，只接受白名单内的 Markdown、源代码、配置或纯文本文件名，可读取连接器服务账号有权读取的任意绝对路径，不依赖工作区根目录或下载设置。连接器会解析规范文件路径，只接受不超过 2 MiB 的普通 UTF-8
...
这些选项默认关闭或保持最小范围。`-AllowAnyFileDownload` 只扩大确认下载范围，不改变位图预览的根目录检查；文本类预览本身已允许任意可读绝对路径。“完全访问权限”还允许 Codex 执行只读预览以外的操作，仍受连接器服务账号的操作系统权限约束。
