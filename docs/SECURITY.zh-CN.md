...
- 浏览器只能通过十分钟单次链接注册，之后使用已批准的 Ed25519 设备密钥认证；不存在共享浏览器
  Token 或恢复登录。
- 连接器同时需要连接器密钥和管理员批准的 Ed25519 设备密钥。
...
- 使用至少 32 字节随机量生成连接器 Token，并把转发服务侧副本保存在仅 root 可读的 `.env`；不要
  把它交给浏览器。
...
2. `BRIDGE_CONNECTOR_TOKEN` 泄露时，替换 Token、重新安装连接器凭据并重启两端。
3. 检查 ECS 和代理日志、浏览器扩展、剪贴板、Shell 历史、CI 输出及相关基础设施凭据。
