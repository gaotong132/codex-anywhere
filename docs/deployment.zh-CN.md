...
这不是零信任转发节点。ECS 会提供 Web 应用、保存角色 Token 和公开信任记录，并强制执行当前协议。
...
路径。证书续期、代理更新和主机安全更新仍由部署者负责。TLS 会在 ECS/VPS 终止；协议 v3 强制浏览器
与连接器之间的每个应用帧进入经过身份验证的端到端加密通道。
...
加密凭据和非敏感配置保存在仓库外的 `%USERPROFILE%\.codex-anywhere`。再次运行安装器会迁移此前
...
协议 v3 刻意不提供滚动兼容或明文回退。ECS 转发服务、其提供的 Web 资源和本地连接器必须保持相同
提交；旧节点会因协议错误被拒绝。请预留一个短暂离线窗口，并协调升级两个运行组件：
