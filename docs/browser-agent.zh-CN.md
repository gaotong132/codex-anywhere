...
PC、Headless 和 steer 消息中，仅在首次相关发送或授权页集合、在线状态变化后附加当前原会话的状态。
状态不变的普通消息不再附加提示，不另发一轮消息，不注入网页内容、URL 或密钥。
...
状态提示只保留实时计数和页面列表入口，单页面从 408 减至 91 字符。消息成功投递后才记录状态，失败或结果不确定
不会消耗提示，也不会触发自动重发。页面数量相同但页面被替换、具体在线页面变化时仍会提醒；撤销授权后的下一条
普通消息提示零页面，异步问题答案保持原封包。Connector 在内存中保留最近 64 个任务的状态，重启或重新访问已淘汰
的授权任务时可能再次提示；执行工具时始终重新核对实时授权，通用说明集中在 MCP 中。
完整升级 v0.3.0 时按发布说明更新 Connector/MCP、Web 与插件，保留已有配对。
...
v0.3.0 已通过 389 项主测试、100 项扩展测试以及类型检查和构建。下列带日期条目保留原始验收数量及指纹，不是当前发布统计，参见[发布验证](release-0.3.0.zh-CN.md#验证与限制)。

- 2026-09-09 状态提示：370 项主测试、类型检查及构建通过，覆盖状态不变时去重、发送失败与回执乱序、数量不变的
  页面替换、在线变化、重连、有界缓存、异步问答封包、全部历史格式及三条消息投递路径；扩展指纹仍为 `f4d6f529`。
