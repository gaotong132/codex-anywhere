...
保持一个手动授权的起始页，不开放任意多页绑定。用户可点击「允许 AI 打开的同站子页」，按需授予
起始站点的可选权限。AI 通过 `open_link`（或点击 `target=_blank` 链接）打开的同源新页签自动归原会话；
手动页签、网站自行 `window.open` 的弹窗、跨源链接/重定向不自动纳管。多个受管页面由模型先列举再指定 `pageId`。
...
用户点击授权时才由外壳读取实际浏览器窗口/标签页；后台在网络等待前捕获文档身份，并在授权前重新验证。
