...
4. 要让会话操作网页，先通过「页面控制设置」完成插件自己的配对，再在侧栏选好聊天会话并点击
   「授权当前页」。Web 与插件各有设备身份，不能重复消费同一条配对链接。

切换聊天会话不会自动把页面授权转移过去；侧栏始终显示实际获授权的环境和会话。切换标签页后若无法授权，
请在目标页再次点击工具栏中的插件图标，取得该页的临时权限，再点击授权。关闭侧栏不主动撤销页面控制；
...
- 选择环境与现有 Session，确认标题后点“授权当前页面”。一个扩展/Session 保持一个手动授权的起始页。
  更多菜单可撤销起始页及子页，再更换会话或页面；不再有实验性“读取页面/停止并清除”。
- 可选点击“允许 AI 打开的同站子页”，Chrome 仅请求当前站点权限；不允许时原单页功能照常使用。
