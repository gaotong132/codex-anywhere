...
指纹取自构建产物和清单内容，与 Git 发布 tag 分开。v0.3.0 仍显示 dev，表示解压加载的实验构建；构建插件本身不会更新服务器。
...
关闭侧栏不主动撤销页面控制；
「撤销页面授权」会撤销起始页及其子页。同源导航和刷新保留标签页同意；跨源导航或关闭页面会撤销授权。
...
## 构建与安装
在其他电脑安装时，把生成的 `extension/dist` 整个目录复制到固定位置，再加载已解压扩展。Git 提交的是源码和构建配置，
不包含此生成目录。核对新安装实例的扩展 ID，将精确 Origin 加入 Relay 白名单，在该浏览器配置中配对聊天一次；
不要复制私钥文件或浏览器配置。原路径升级只需重新加载插件，保留已有配对。参见 [v0.3.0 升级说明](../docs/release-0.3.0.zh-CN.md#升级)。
...
真实鼠标操作需要扩展声明 `debugger` 权限；Chrome 不支持将它作为可选权限。
重新加载或启用新版扩展时，如 Chrome 提示新增权限，请确认该提示。
Chrome 可能在操作期间显示调试提示。扩展只连接已授权的标签页，发送固定的移动、按下、抬起事件。
成功点击与截图复用同一已授权文档的调试连接，空闲 60 秒后释放，避免提示条反复改变布局、关闭下拉菜单。
导航、撤销、断线或操作失败时提前释放；用户取消 Chrome 调试提示也会撤销该页授权。
模型不能传入调试命令或任意坐标；该权限不授权其他标签页，不关闭弹窗保护，也不接管已被其他调试器占用的页面。
浏览器策略拒绝或调试器冲突时明确报告，不退回脚本点击。点击过程允许控件的悬停、聚焦样式变化；
控件替换、文字或动作属性变化以及遮挡仍会阻断点击。
...
- 在聊天中选择环境与现有 Session，确认侧栏上方标题后点“授权当前页”。一个扩展/Session 保持一个手动授权的起始页。
  明确授权新页可按上述规则替换本插件的旧起始页；更多菜单可撤销起始页及子页。只切换聊天任务不会自动转移授权。
...
- **授权没有 10 分钟限制**。普通操作最多 15 秒；截图捕获最多 15 秒，含图片传输的整体等待最多 60 秒。20 秒心跳维持连接；超过 45 秒未收到心跳显示离线。
  正文和操作通过现有端到端加密通道传输，Relay 不读取内容。
- 关闭弹窗不撤销授权。后台 worker 重启/网络重连只恢复当前浏览器生命周期中、相同 Session、相同文档的同意，
  且旋转授权 ID；单页应用的 URL 变化仅在原 documentId/来源校验通过时保留授权，已有站点权限时，同源导航/刷新续接新文档并废弃旧 pageId、引用及在途结果；跨源导航、撤销、标签页关闭或浏览器重启后须重新授权。
  网络断开时不会重放点击/输入；超时操作可能已执行，先读取页面确认，不能盲目重试。
- 起始页跨源导航/关闭/撤销会停止所有子页的纳管；子页跨源导航仅撤销自身。连接器重启若丢失子页来源记录，
...
- 工具只使用当前快照生成的元素引用，输入/点击后旧引用失效。不支持任意 JavaScript、Cookie/密码导出、
  文件上传、浏览器内部页、iframe/shadow DOM 内部控件、canvas 坐标操作、原生弹窗或桌面控制。截图可查看可见 canvas。可见正文仍可能含敏感信息，
...
更新插件、Connector 和 MCP 后，已授权页面默认支持截图，无需额外开关、再次配对或选择会话。
页面授权同时包含文字读取与截图；撤销页面授权即可停止对该页面的访问。
截图沿用真实鼠标点击所需的 `debugger` 权限，Chrome 更新提示需正常确认，不额外申请全站权限。
新增工具 `anywhere_browser_screenshot({pageId})` 按当前任务的授权精确定位标签页，后台页也不会改截活动页。
只有一个授权页时可省略 `pageId`；截图不会增加授权或切换标签页。

仅截可见视口，返回最大边长 1920 像素、最多 1 MiB 的 JPEG 图片给模型。不会滚动拼接整页，
也不会截到地址栏、侧栏聊天或桌面。输入框、嵌入文档、可检测的开放 shadow host、私密标记区域会在
插件内遮挡后才经端到端加密通道发送；普通正文、图片或 canvas 仍可能包含敏感内容，不保证识别全部秘密。
页面/保护区域在捕获期间变化、导航、撤销授权、调试器冲突或尺寸超限时返回具体错误，不发送截图。
高倍率页面缩放、保护内容和持续变化的页面可能暂不支持，可继续使用文字快照。

模型可用截图理解图表或核对结果；点击/填写仍需最新文字快照中的 ref。新增 MCP 工具需重新加载 Codex 的
MCP 配置；如 Desktop 尚未发现它，请在任务空闲时重启应用。Relay 不持久存储截图；Codex 可能按其正常任务历史规则保存工具图片。

## 控制台操作与诊断

表单操作沿用原有工具，截图用于补充视觉信息。父容器汇总标签不再吞掉内部文本不同的独立菜单选项，装饰文字与图标仍会去重。快照把按钮内嵌文字和可见标签关联到可操作的 ref，并按需提供 `role`、`inputType`、
...
真实鼠标点击遵循浏览器的正常指针和聚焦行为；填写时聚焦但不滚动，也不会自动提交表单。
成功点击和截图会在同一已授权文档内复用调试连接，空闲60秒后释放，避免Chrome调试提示条反复改变窗口高度、关闭下拉菜单。
导航、撤销授权、断线、用户取消或操作失败时释放连接，不接管其他客户端占用的调试器。
用户取消Chrome调试提示时同时撤销该页授权，后续调用不能静默重新连接。
快照可附带最多4条 `excludedVisibleBranches`，仅记录标签类型、排除原因和节点位置，不返回被排除区域的文字、属性或表单值，用于区分组件漏读与私有区域。
点击位置取视口/面板裁剪后的可见文字片段，避免把部分露出的控件或换行链接的整体中心误判为遮挡。
真正被其他元素盖住的控件仍返回 `browser_element_obscured`，不会穿透覆盖层点击。
按下、抬起前重新核对精确文档、元素引用、可见位置和当前授权。按下后无法确认完成时返回
`browser_native_click_interrupted`，必须先检查页面再决定是否重试。鼠标事件已发送不等于应用操作已成功。
...
iframe/shadow DOM 内控件、canvas 坐标操作、文件上传和原生对话框仍不支持；截图可读取可见的 canvas 图像，遇到时应明确报告插件能力缺口，重新配对不能解决。
...
v0.3.0 的 389 项主测试、100 项扩展测试、类型检查和构建均已通过。下列带日期的浏览器验收保留原有范围；发布指纹和升级步骤参见[发布说明](../docs/release-0.3.0.zh-CN.md#验证与限制)。
...
参见[架构与验收清单](../docs/browser-agent.zh-CN.md)与[阶段审视和重构记录](docs/refactor-2026-09-09.zh-CN.md)。
