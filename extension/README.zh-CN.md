...
点击位置取视口/面板裁剪后的可见文字片段，避免把部分露出的控件或换行链接的整体中心误判为遮挡。
真正被其他元素盖住的控件仍返回 `browser_element_obscured`，不会穿透覆盖层点击。
...
- `scroll` 可传入快照中 `scrollable` 面板的 `ref`，省略时滚动整页。面板快照补充可滚动方向
  `scrollAxes`（`x`/`y`）及当前位置 `scrollPosition`（`x`、`y`）。每个方向的位移为 ±2000 像素内的整数：
