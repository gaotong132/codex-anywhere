...
Application source and tests are written in TypeScript. The Node relay and connector are compiled
with `tsconfig.node.json`; the Windows launcher rebuilds only when TypeScript source is newer, then
runs the compiled connector as a single Node process. `tsx` is limited to the test runner. Runtime
browser configuration is generated by the relay at `/config.js`, so it does not require a checked-in
JavaScript source file.

The React entry component delegates conversation/history parsing to `history-utils.ts`, image work
to `image-utils.ts`, local-link decoding to `file-utils.ts`, shared UI to `ui-components.tsx`, and
pure presentation helpers to `app-utils.ts`. Protocol behavior lives under `src/server` and
`src/connector`.
